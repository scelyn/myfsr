    ...
    <div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <form action="{{ route('invoices.index') }}" method="GET" class="w-full md:w-96 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari No Invoice atau Nama Toko..." class="w-full pl-10 pr-4 py-2.5 bg-theme-card border border-theme-border text-theme-text1 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all outline-none shadow-md shadow-black/20">
            </form>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($invoices as $invoice)
                            <tr class="hover:bg-theme-bg/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right font-black {{ $invoice->remaining_amount > 0 ? 'text-theme-errorText' : 'text-theme-successText' }}">
                                    Rp {{ number_format($invoice->remaining_amount, 0, ',', '.') }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($invoice->status == 'paid')
                                        <span class="inline-block px-3 py-1 bg-theme-success/40 text-theme-successText text-[10px] font-black rounded-full uppercase tracking-widest border border-emerald-200">LUNAS</span>
                                    ...
                                    @else
                                        <span class="inline-block px-3 py-1 bg-theme-error/40 text-theme-errorText text-[10px] font-black rounded-full uppercase tracking-widest border border-red-200">BELUM DIBAYAR</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('invoices.show', $invoice) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-theme-sidebar text-theme-text2 hover:bg-theme-success/20 hover:text-theme-successText transition-colors" title="Lihat Detail">
