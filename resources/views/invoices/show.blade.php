<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('orders.index') }}" class="text-sm font-medium text-theme-text2 hover:text-theme-successText flex items-center gap-2">
            ...
            <div class="flex items-center gap-3 relative" x-data="{ printOpen: false }">
                ...
                @endphp
                <a href="{{ $waUrl }}" target="_blank" class="px-4 py-2 bg-theme-border text-theme-text1 hover:bg-theme-text2 hover:text-theme-sidebar rounded-xl text-sm font-bold flex items-center gap-2 shadow-md shadow-black/20 transition-colors">
        ...
        <!-- Invoice Paper -->
        <div class="bg-theme-card p-12 rounded-2xl border border-slate-50 shadow-md shadow-black/20 print:shadow-none print:border-none print:p-0">
            @if($previous_tunggakan > 0)
            ...
            <div class="flex justify-between items-start border-b border-theme-border pb-8 mb-8">
                ...
                <div class="text-right">
                    ...
                    @if($invoice->status == 'paid')
                        <span class="inline-block mt-4 px-4 py-1.5 bg-theme-success/40 text-theme-successText text-xs font-black rounded-full uppercase tracking-widest border border-emerald-200">LUNAS</span>
                    ...
                    @else
                        <span class="inline-block mt-4 px-4 py-1.5 bg-theme-error/40 text-theme-errorText text-xs font-black rounded-full uppercase tracking-widest border border-red-200">BELUM DIBAYAR</span>
                    @endif
                </div>
            </div>
                    ...
                    @foreach($invoice->order->items as $item)
                        <tr>
                            ...
                            <td class="py-4 text-center">
                                <p class="text-sm font-medium text-theme-text1">{{ number_format($item->quantity) }} <span class="text-xs text-theme-text2">{{ $item->product_unit }}</span></p>
                            </td>
            ...
            <div class="flex flex-col items-end border-t border-theme-border pt-8 mt-8">
                <div class="w-full max-w-[320px]">
                    ...
                    <div class="space-y-4">
                        ...
                        @if($previous_tunggakan > 0)
                        <div class="flex justify-between items-center pb-2 border-b border-dashed border-[#4A5C6A]">
                        ...
                        @if($invoice->paid_amount > 0)
                            ...
                            <div class="flex justify-between items-center py-2 border-t border-theme-border mt-2">
                                <span class="text-base font-black text-theme-errorText">SISA TAGIHAN (Nota Ini)</span>
                                <span class="text-xl font-black text-theme-errorText">Rp {{ number_format($invoice->remaining_amount, 0, ',', '.') }}</span>
                            </div>
                        @endif
                    </div>
                </div>
    ...
    <div id="payment-section" class="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl mx-auto print:hidden">
        ...
        <!-- Form Pembayaran -->
        <div class="bg-theme-card rounded-2xl border border-theme-border shadow-md shadow-black/20 p-8">
            ...
            @if(session('success'))
                <div class="mb-6 p-4 bg-theme-success/20 text-theme-successText rounded-xl text-sm font-bold border border-theme-success flex items-center gap-3">
            ...
            @if(session('error'))
                <div class="mb-6 p-4 bg-theme-error/20 text-theme-errorText rounded-xl text-sm font-bold border border-theme-error flex items-center gap-3">
            ...
            @if($invoice->status == 'paid')
                <div class="p-6 bg-theme-bg border border-theme-border rounded-2xl text-center">
                    <div class="w-12 h-12 bg-theme-success/40 text-theme-successText rounded-full flex items-center justify-center mx-auto mb-3">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                    </div>
                    <p class="font-bold text-theme-text1">Tagihan Sudah Lunas</p>
                    <p class="text-xs text-theme-text2 mt-1">Tidak ada sisa piutang untuk nota ini.</p>
                </div>
            @else
                <form action="{{ route('payments.store') }}" method="POST" class="space-y-5" x-data="{ amount: {{ $invoice->remaining_amount }} }">
                    @csrf
                    ...
                    <div>
                        <label class="block text-xs font-black text-theme-text2 uppercase tracking-widest mb-2">Sisa Piutang Saat Ini</label>
                        <div class="text-2xl font-black text-theme-text1">Rp {{ number_format($invoice->remaining_amount, 0, ',', '.') }}</div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input 
                                ...
                                class="w-full pl-12 pr-4 py-3 bg-theme-bg border border-theme-border text-theme-text1 rounded-xl text-lg font-black focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all outline-none shadow-md shadow-black/20"
                                required
                            >
                        </div>
                        <p class="text-[10px] font-bold text-theme-text2 mt-2">Maksimal: Rp {{ number_format($invoice->remaining_amount, 0, ',', '.') }}</p>
                    </div>

                    <div>
                        <label class="block text-xs font-black text-theme-text2 uppercase tracking-widest mb-2">Catatan (Opsional)</label>
                        <input type="text" name="notes" placeholder="Misal: Cicilan ke-1" class="w-full px-4 py-3 bg-theme-card border border-theme-border text-theme-text1 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all outline-none shadow-md shadow-black/20">
                    </div>
                ...
                <div class="space-y-4">
                    @foreach($invoice->payments()->latest()->get() as $payment)
                        <div class="p-4 border border-theme-border bg-theme-bg rounded-2xl flex items-center justify-between">
                            ...
                            <div class="text-right">
                                <p class="text-base font-black text-theme-successText">+ Rp {{ number_format($payment->amount, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
