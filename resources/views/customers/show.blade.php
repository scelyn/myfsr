<x-admin-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <a href="{{ route('customers.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-theme-text2 hover:text-white transition-colors group">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-4">
                {{-- Profile Card --}}
                <div class="bg-slate-100 rounded-2xl p-6 text-theme-text1">
                    ...
                    <p class="text-theme-text2 text-sm mt-1">{{ $customer->nama_pemilik }}</p>
                    <div class="mt-4 pt-4 border-t border-white/10 space-y-2">
                        <div class="flex items-center gap-2 text-sm text-theme-text1">
                        ...
                        <div class="flex items-start gap-2 text-sm text-theme-text1">
                            <svg class="w-4 h-4 mt-0.5 shrink-0 text-theme-text2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="bg-theme-card rounded-2xl border border-theme-border p-4">
                        ...
                        <p class="text-2xl font-black text-white mt-1">{{ $customer->orders->count() }}</p>
                    </div>
                    <div class="bg-theme-card rounded-2xl border border-theme-border p-4">
                        <p class="text-[10px] font-bold text-theme-text2 uppercase tracking-wider">Pembayaran</p>
                        <p class="text-2xl font-black text-white mt-1">{{ $customer->customerPayments->count() }}</p>
                    </div>
                    <div class="bg-theme-card rounded-2xl border border-theme-border p-4 col-span-2">
                        <p class="text-[10px] font-bold text-rose-600 uppercase tracking-wider">Piutang Aktif</p>
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-theme-card rounded-2xl border border-theme-border shadow-md shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-theme-border flex items-center justify-between">
                        <h3 class="text-sm font-bold text-white">Histori Pesanan</h3>
                        <a href="{{ route('orders.index') }}?customer={{ $customer->id }}" class="text-xs font-semibold text-emerald-600 hover:underline">Lihat Semua</a>
                    </div>
                    <div class="divide-y divide-slate-50">
                        @forelse($customer->orders as $order)
                            <div class="flex items-center justify-between px-6 py-4 hover:bg-theme-bg/50 transition-colors">
                                <div>
                                    <a href="{{ route('orders.show', $order) }}" class="text-sm font-bold text-[#11212D] hover:text-emerald-600 transition-colors">
                                        {{ $order->order_number }}
                                    </a>
                                    <p class="text-xs text-theme-text2 mt-0.5">{{ $order->order_date->format('d M Y') }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-bold text-white">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</p>
                ...
                <div class="bg-theme-card rounded-2xl border border-theme-border shadow-md shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-theme-border">
                        <h3 class="text-sm font-bold text-white">Status Piutang</h3>
                    </div>
                    <div class="divide-y divide-slate-50">
                        @forelse($customer->receivables as $receivable)
                            <div class="flex items-center justify-between px-6 py-4 hover:bg-theme-bg/50 transition-colors">
                                ...
                                <div class="text-right">
                                    <p class="text-sm font-bold text-rose-600">Rp {{ number_format($receivable->remaining_amount, 0, ',', '.') }}</p>
                                    <span class="inline-block mt-0.5 px-2 py-0.5 text-[10px] font-bold rounded-full uppercase
                                        {{ $receivable->status === 'paid' ? 'bg-theme-success/40 text-emerald-600' : ($receivable->status === 'partial' ? 'bg-blue-100 text-blue-700' : 'bg-theme-error/40 text-rose-600') }}">
                                        {{ $receivable->status }}
                                    </span>
                                </div>
                            </div>
                ...
                <div class="bg-theme-card rounded-2xl border border-theme-border shadow-md shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-theme-border">
                        <h3 class="text-sm font-bold text-white">Histori Pembayaran</h3>
                    </div>
                    <div class="divide-y divide-slate-50">
                        @forelse($customer->customerPayments as $payment)
                            <div class="flex items-center justify-between px-6 py-4">
                                ...
                                <p class="text-sm font-black text-emerald-600">+ Rp {{ number_format($payment->amount, 0, ',', '.') }}</p>
                            </div>
                        @empty
