<x-admin-layout>
    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-theme-card p-8 rounded-2xl border border-slate-50 shadow-md shadow-black/20">
            ...
            <div class="bg-theme-card p-8 rounded-2xl border border-slate-50 shadow-md shadow-black/20 border-l-4 border-l-red-500">
                <p class="text-[10px] font-black text-theme-errorText uppercase tracking-widest">Jatuh Tempo (Overdue)</p>
                <div class="text-3xl font-black text-theme-errorText mt-2">Rp {{ number_format($stats['total_overdue'], 0, ',', '.') }}</div>
            </div>
        ...
        <!-- Filter Card -->
        <div class="bg-theme-card p-6 rounded-2xl border border-slate-50 shadow-md shadow-black/20">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($receivables as $r)
                            <tr class="hover:bg-theme-bg/30 transition-colors">
                                ...
                                <td class="px-8 py-5">
                                    @if($r->status === 'paid')
                                        <span class="px-3 py-1 bg-theme-success/40 text-theme-successText text-[10px] font-black rounded-full uppercase tracking-widest">LUNAS</span>
                                    ...
                                    @else
                                        <span class="px-3 py-1 bg-theme-error/40 text-theme-errorText text-[10px] font-black rounded-full uppercase tracking-widest">BELUM BAYAR</span>
                                    @endif
                                </td>
                                <td class="px-8 py-5 text-right">
                                    <a href="{{ route('receivables.show', $r) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-theme-bg text-theme-text1 hover:bg-theme-success/20 hover:text-theme-successText text-xs font-bold rounded-xl transition-all">
