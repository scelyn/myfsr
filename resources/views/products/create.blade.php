<x-admin-layout>
    ...
    <div class="max-w-2xl">
        ...
        <form action="{{ route('products.store') }}" method="POST"
              x-data="{ harga_beli: 0, margin: 0 }">
            @csrf
            <section class="card shadow-card">
                <div class="card-header"><h3>Informasi Produk Baru</h3></div>
                <div class="p-6 space-y-5">
                    ...
                    <hr class="divider">
                    <p class="text-xs font-bold uppercase tracking-widest" style="color:var(--text-muted);">Harga &amp; Margin Default</p>
                        ...
                        <div class="form-group">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="harga_beli_default" x-model.number="harga_beli"
                                       value="{{ old('harga_beli_default', 0) }}" min="0" step="500"
                                       class="form-input pl-9 {{ $errors->has('harga_beli_default') ? 'error' : '' }}">
                            </div>
                            @error('harga_beli_default')<p class="text-xs" style="color:var(--color-danger);">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="margin_default" x-model.number="margin"
                                       value="{{ old('margin_default', 0) }}" min="0" step="500"
                                       class="form-input pl-9 {{ $errors->has('margin_default') ? 'error' : '' }}">
                            </div>
                            @error('margin_default')<p class="text-xs" style="color:var(--color-danger);">{{ $message }}</p>@enderror
                        </div>
                    </div>
