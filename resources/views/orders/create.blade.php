                    ...
                    <div class="bg-theme-card rounded-2xl border border-theme-border shadow-md shadow-sm p-8 transition-all hover:shadow-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2" wire:ignore>
                                <label class="text-xs font-bold text-theme-text2 uppercase tracking-widest">Pilih Customer <span class="text-rose-600">*</span></label>
                            ...
                            <div class="space-y-2">
                                <label class="text-xs font-bold text-theme-text2 uppercase tracking-widest">Tanggal Transaksi <span class="text-rose-600">*</span></label>
                                <input type="date" name="order_date" value="{{ old('order_date', date('Y-m-d')) }}" class="w-full px-4 py-3 bg-transparent border border-slate-300 text-theme-text1 rounded-xl text-sm focus:ring-2 focus:ring-theme-primary/20 focus:border-theme-primary transition-all outline-none" required>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-0 overflow-x-auto">
                            <table class="w-full text-left">
                                <thead class="bg-slate-50">
                                    <tr>
                                        <th class="px-8 py-3 text-[10px] font-bold text-theme-text2 uppercase tracking-widest w-1/2 border-b border-theme-border">Produk</th>
                                        <th class="px-4 py-3 text-[10px] font-bold text-theme-text2 uppercase tracking-widest w-32 text-center border-b border-theme-border">Qty</th>
                                        <th class="px-8 py-3 text-[10px] font-bold text-theme-text2 uppercase tracking-widest text-right border-b border-theme-border">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-theme-border">
                                    <template x-for="(item, index) in items" :key="item.id">
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="px-8 py-5 text-right">
                                                <button type="button" @click="removeItem(index)" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-theme-error/10 rounded-lg transition-all">
                                    ...
                                    <template x-if="items.length === 0">
                                        <tr>
                                            <td colspan="5" class="px-8 py-10 text-center text-slate-400 italic text-sm border-b border-theme-border">
                ...
                <div class="lg:col-span-1 space-y-8">
                    ...
                    <div class="bg-theme-card rounded-2xl border border-theme-border shadow-md shadow-sm overflow-hidden sticky top-8">
                        ...
                        <div class="p-8 space-y-6">
                            <div class="flex justify-between items-center pb-4 border-b border-theme-border">
                                <span class="text-sm font-medium text-theme-text2">Total Produk</span>
                                <span class="text-sm font-bold text-theme-text1" x-text="items.length"></span>
                            </div>
                            <div class="flex justify-between items-center pb-4 border-b border-theme-border">
                                <span class="text-sm font-medium text-theme-text2">Total Qty</span>
                                <span class="text-sm font-bold text-theme-text1" x-text="items.reduce((sum, item) => sum + (parseFloat(item.quantity) || 0), 0)"></span>
                            </div>
