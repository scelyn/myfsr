...
<a {{ $attributes->merge(['class' => $classes]) }} :title="sidebarCollapsed ? '{{ addslashes(trim($slot)) }}' : ''" :class="sidebarCollapsed ? 'lg:justify-center' : ''">
    ...
    <span class="whitespace-nowrap lg:hidden">{{ $slot }}</span>
    
    <!-- Desktop Text (Hidden when collapsed) -->
    <span x-show="!sidebarCollapsed" x-transition.opacity.duration.200ms class="whitespace-nowrap hidden lg:block">{{ $slot }}</span>
</a>
