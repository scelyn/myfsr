@if (session()->has('success'))
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" 
         class="mb-6 flex items-center p-4 text-emerald-800 border-t-4 border-emerald-300 bg-theme-success/20 rounded-xl shadow-md shadow-black/20" role="alert">
...
@if (session()->has('error'))
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)" 
         class="mb-6 flex items-center p-4 text-red-800 border-t-4 border-red-300 bg-theme-error/20 rounded-xl shadow-md shadow-black/20" role="alert">
        <svg class="flex-shrink-0 w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
